...
for estado in niveis_co2:
    # Aqui sabemos quantos sensores há em cada estados, mas vamos supor que não
    qtde_sensores = len(niveis_co2[estado])
    soma_co2 = sum(niveis_co2[estado])
    media_co2 = soma_co2 / qtde_sensores
...
for info in video:
   # Aqui, verificaremos as chaves do dicionário
   # Temos que ter pelo menos uma informação dentro do for
   print(info)
   # Identificando a chave que possui a informação que queremos, iremos printar seu conteúdo
   # print(video["download"])
   # Dentro no valor da chave há uma lista com dicionários
   # print(video['download'][0])
   # Agora poderemos acessar o dicionário link e seu valor:
